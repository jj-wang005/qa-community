...
def test_unlike_decrements_like_count(client, auth, db_session):
    ...
    client.post(f"/like/{aid}", headers={"Authorization": f"Bearer {token}"})
    db_session.commit()
    assert db_session.get(Answer, aid).like_count == 1

    resp = client.delete(f"/like/{aid}", headers={"Authorization": f"Bearer {token}"})
